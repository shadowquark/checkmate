...
// Coded by yyFish ############################################################# 
	...
	electronsLoose = overlapRemoval( electronsLoose , 0.1 );
	TLorentzVector dilep;
	if ( muonsCombined.size() == 2 && electronsLoose.size() == 1 &&\
		! ( muonsCombined[0]->Charge + muonsCombined[1]->Charge ) )
	{
		if ( muonsCombined[0]->PT < 20 || muonsCombined[1]->PT < 10 ||\
			electronsLoose[0]->PT < 25 )	return;
		flag3l = 0;
		dilep = muonsCombined[0]->P4() + muonsCombined[1]->P4();
	}
	if ( muonsCombined.size() == 1 && electronsLoose.size() == 2 &&\
		! ( electronsLoose[0]->Charge + electronsLoose[1]->Charge ) )
	{
		if ( muonsCombined[0]->PT < 20 || electronsLoose[1]->PT < 15 ||\
			electronsLoose[0]->PT < 25 )	return;
		flag3l = 1;
	...
	if (flag3l)
		...
		Mt = mT( electronsLoose[0]->P4() , missingET->P4() );
