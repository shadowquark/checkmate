...
void Atlas_1709_07703_13::initialize() {
  setAnalysisName("atlas_1709_07703_13");          
  setInformation(""
    "# atlas 1709.07703 13TeV 36.1/fb 4mu\n"
  "");
...
void Atlas_1709_07703_13::analyze() {
	...
	for ( int i = 0 ; i < muonsCombined.size() ; ++ i )
		for ( int j = 0 ; j < muonsCombined.size() ; ++ j )
			for ( int k = 0 ; k < muonsCombined.size() ; ++ k )
				for ( int kk = 0 ; kk < muonsCombined.size() ; ++ kk )
					if ( muonsCombined[i]->Charge + muonsCombined[j]->Charge
						+ muonsCombined[k]->Charge + muonsCombined[kk]->Charge == 0 
						&& muonsCombined[i]->PT > muonsCombined[j]->PT
						&& muonsCombined[j]->PT > muonsCombined[k]->PT 
						&& muonsCombined[k]->PT > muonsCombined[kk]->PT 
						&& muonsCombined[i]->PT > 20 
						&& muonsCombined[j]->PT > 15
						&& muonsCombined[k]->PT > 10 )
					{
						if ( muonsCombined[i]->Charge + muonsCombined[j]->Charge == 0 )
						{
